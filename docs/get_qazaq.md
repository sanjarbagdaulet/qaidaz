Документация
Логика классификации not_kz и расширения графа каналов
1. Статусы not_kz
Значение	Название	Описание
0	unknown	Канал не классифицирован
1	blocked	Точно не казахстанский, жёсткая граница
2	allowed	Не определено как не-KZ (разрешён)
3	legacy	Историческая ручная метка (география неясна)
4	quarantine	Автоматический карантин (неясно)
5	cand_1	Кандидат в not_kz=1
6	cand_2	Кандидат в not_kz=2

Примечания:

3 больше не присваивается автоматически и используется только как историческая метка.

Рабочие автоматические статусы: 1,2,4,5,6.

2. Endlist (внешние списки)

Существуют три внешних списка ID:
...
endlist(channel_id BIGINT PRIMARY KEY, mark TINYINT)

Правило обработки

При обнаружении channel_id из endlist в get_channels:

Устанавливается channels.not_kz = mark

Запись удаляется из endlist

Канал исключается из дальнейшего пайплайна

endlist имеет абсолютный приоритет.

3. Центр (CORE)

CORE — это каналы, которые одновременно:

kk_ratio_by_f > 20

not_kz = 2

По всем каналам CORE обязательно выполняется запрос похожих.

4. Расширение графа (ограниченная глубина 2)

Расширение выполняется в два шага и строго ограничено.

Волна 1 (CORE → R1)

Для каждого канала из CORE:

выполняется запрос похожих

полученные каналы образуют множество R1

сохраняются связи recs(core, r1)

Волна 2 (R1 → R2)

Для каждого канала r1 ∈ R1, полученного только от CORE:

выполняется запрос похожих

полученные каналы образуют множество R2

сохраняются связи recs(r1, r2)

Ограничение

По каналам из R2 запрос похожих не выполняется

Глубина расширения строго ограничена двумя волнами

Пример
CORE: qazaqstantv

R1: ztb, tengrinews, kozachkow
  → по ним делаем запрос похожих
...
По orda, almatv, astanovka, glego, egovpress, neftegaz
запрос похожих НЕ выполняется

5. Граница и защита от утечек

Каналы с not_kz=1:

никогда не используются как seed

по ним не выполняются запросы похожих

Каналы вне CORE:

не могут быть seed’ами первой волны

Каналы R2:

никогда не становятся seed’ами

Инвариант: расширение возможно только от каналов CORE и их прямых рекомендаций.

6. Назначение документа

Этот документ:

фиксирует текущую модель мышления и инварианты

служит основой для реализации пайплайна и SQL

используется как точка отсчёта для последующих изменений
